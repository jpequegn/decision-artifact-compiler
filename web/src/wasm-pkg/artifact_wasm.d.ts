 ...
 */
export function diff_artifacts(base: string, current: string): any;
 ...
 */
export function validate_artifact(source: string): any;

export type InitInput = RequestInfo | URL | Response | BufferSource | WebAssembly.Module;

export interface InitOutput {
    readonly memory: WebAssembly.Memory;
    readonly diff_artifacts: (a: number, b: number, c: number, d: number) => [number, number, number];
    readonly validate_artifact: (a: number, b: number) => [number, number, number];
